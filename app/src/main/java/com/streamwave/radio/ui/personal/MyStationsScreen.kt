...
fun MyStationsScreen(onBack: () -> Unit) {
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text("Mijn stations", color = PrimaryText) },
                navigationIcon = {
        ...
        Box(modifier = Modifier.fillMaxSize().padding(padding), contentAlignment = Alignment.Center) {
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                Text("Nog geen persoonlijke stations", color = SecondaryText)
                Text("Voeg een station toe via de + knop op het hoofdscherm", color = SecondaryText)
            }
        }
    }
}
